...
- **`configs/` holds exactly five tracked files**: `hybrid_moe_lineup.toml`, `local_gguf_lineup.toml`, `local_safetensors_lineup.template.toml`, `model_adapter_configs.toml`, and `saaq_cloud_lineup.toml` (`safetensors_lineup.toml` is a gitignored local copy made from the template). Docs and the `justfile` cited three earlier names — `saaq15_moe_lineup.toml`, `saaq15_cloud_lineup.toml`, `safetensors_lineup.template.toml` — that have not existed for some time; those are now corrected, so check `configs/` before reintroducing a path. `LINEUP_CONFIG` is deliberately left undefaulted by `just saaq-campaign`: pointing it at a missing file aborts the run, because set-but-unloadable is a hard error, whereas unset falls through to `SAFETENSORS_LINEUP_CONFIG` → `CHECKPOINT_PATH` → autodiscovery.
