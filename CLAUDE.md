...
- **`configs/` tracks five files as of this writing** — `hybrid_moe_lineup.toml`, `local_gguf_lineup.template.toml`, `local_safetensors_lineup.template.toml`, `model_adapter_configs.toml`, `saaq_cloud_lineup.toml` — but list the directory rather than trusting this sentence; it has drifted before. The two `*.template.toml` files are copied to gitignored local names (`local_gguf_lineup.toml`, `safetensors_lineup.toml`) and filled in with machine paths; AGENTS.md forbids committing local absolute paths, so never `git add -f` those copies. `model_adapter_configs.toml` is reference metadata that no code loads — `RunMatrix` deserializes `[[run]]` only. Docs and the `justfile` cited three earlier names — `saaq15_moe_lineup.toml`, `saaq15_cloud_lineup.toml`, `safetensors_lineup.template.toml` — that have not existed for some time; those are now corrected, so check `configs/` before reintroducing a path. `just saaq-campaign` **requires** `LINEUP_CONFIG` and checks it before phase 1. It must not be defaulted (set-but-unloadable is a hard error) and must not be left to fall through either: autodiscovery's candidate list includes the dense `glm46v_flash_q8_0`, which `resolve_gguf_topology` rejects for missing `expert_count`, aborting the sweep. A baseline campaign also has to pin its model set to be comparable across phases.
